...
public class DBHelper_CP extends SQLiteOpenHelper {
    ...
    public static final String DB_NAME = "IMcp.db";
    public static final String USER_TABLE = "company_payments";
    ...
    public void addSupplierBill(String login_id, String bill_no, String supName, int tamt, int pamt, String status) {
    ...
    public boolean updateData(String login_id, String bill_no, String supName, int tamt, int pamt, String status) {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(LOGIN_ID, login_id);
        values.put(TOTAL_AMOUNT, tamt);
        values.put(PAYED_AMOUNT, pamt);
        values.put(STATUS, status);
        int res = sqLiteDatabase.update(USER_TABLE, values, "login_id=? and bill_no=? and supplier_name=?", new String[]{login_id,bill_no,supName});
        if (res > 0) {
            return true;
        } else {
            return false;
